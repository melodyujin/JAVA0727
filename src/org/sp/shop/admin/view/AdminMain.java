...
public class AdminMain extends JFrame{
		...
		for(int i=0;i<navi.size();i++) {
			...
			obj.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					int index=navi.indexOf(e.getSource()); //이벤트를 일으킨 JLable이 몇번째 들어있는지 조사
					showHide(index);
				}
			});
		}
	...
	public void login() {
		...
		this.setVisible(true); //현재 메인프레임을 다시 보이게..
	...
	public void logout() {
		...
		loginForm.setVisible(true); //로그인폼 다시 보이게..
	}
	
	//페이지 전환처리
	public void showHide(int n) { //보이게하고 싶은 index만 넘겨받는다
		for(int i=0;i<pages.length;i++){
			if(i==n) { //넘겨받은 매개변수와 i가 일치할 때만 보이게..
